 ...
 *
 *  Note that RegionEvents are not transactional.  They are logged by this 
 *  listener, but no validation is performed and counters are not modified 
 *  upon processing RegionEvents.
 * 
 *  @author lhughes
 */
public class TestListener extends util.AbstractListener implements CacheListener {
...
private boolean handleEntryEvent(String opName, EntryEvent event) {
  ...
  Log.getLogWriter().fine("TestListener: op from event = " + op.toString());
