 ...
 * @since 6.0
 */
public class MixedRegionWriter extends util.AbstractWriter implements CacheWriter, Declarable {
...
public void beforeRegionDestroy(RegionEvent event) {
   ...
   if (!event.getOperation().equals(Operation.CACHE_CLOSE)) {
      ...
      try {
        ...
        throwException("MixedRegionWriter: beforeRegionDestroy() caught " + e + " during beforeRegionDestroy");
      }
   }
}
...
public void beforeRegionClear(RegionEvent event) {
   ...
   try {
      ...
      throwException("MixedRegionWriter: beforeRegionClear() caught " + e + " during beforeRegionClear");
   }
}
...
protected void checkCallback(CacheEvent event, String expectedCallbackPrefix) {
   ...
   if (callbackObj != null) {  // callbacks can be null with ConcurrentMap ops
