 ...
 * cases with conflation enabled
 * 
 * @author Dinesh Patel
 * @author Mitul Bid
 * 
 */
public class HANoFailoverConflationEnabledClientListener extends
    CacheListenerAdapter
{
   ...
   * validate that the new value is more than the previous value.<br>
  ...
  public void afterCreate(EntryEvent event)
  {
    ...
    if (value.longValue() != 0) {
      validateIncrementedNewValue(key, value);
    }
   ...
   * more than that in the <code>latestValues</code> map. If the oldValue in
   * <code>latestValues</code> map is null or the above validation fails,
  ...
  private void validateIncrementedNewValue(String key, Long newValue)
  {
    ...
    if (diff <= 0) {
      throwException("newValue must be greater than oldValue , but it was not for key = "
          + key + " & newVal = " + newValue + " oldValue = " + oldValue);
      return;
    }
  }
