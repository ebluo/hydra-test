 ...
 * @since 4.2.3
 */
public class SingleKeyListener extends util.AbstractListener implements CacheListener, Declarable {
 ...
 *         false otherwise.
 */
public SingleKeyListener(boolean isCarefulValidation) {
...
public void afterCreate(EntryEvent event) {
   logCall("afterCreate", event);
	...
	protected void checkKey(EntryEvent event) {
           // always allow locally generated events
           if (event.isOriginRemote() == true) {
	      if (!event.getKey().equals( this.key) ) {
	        throwException("Received event " + event.toString() + " for entry " +  event.getKey() + " but this region configured to receive updates for " + this.key + " only ");
	      }
           }
	}
	...
	public void setSingleKey( Object key ) {
          Log.getLogWriter().info("Listener expects updates on " + key + " + only!!!");
	  this.key = key;
        }
