 ...
 *
 *  verifies "isPutAll" flag for putAll operations (so this is typically used to 
 *  override hct.OperationListener when debugging putAll tests (where operations have
 *  been restricted to putAll ops
 *
 ...
 * @since 6.5
 */
public class PutAllListener extends util.AbstractListener implements CacheListener, Declarable {
...
public void afterCreate(EntryEvent event) {
   ...
   if (!event.getOperation().isPutAll()) {
     throwException("afterCreate invoked for " + event.getKey() + ", but Operation.isPutAll() returns false");
   }
...
public void afterUpdate(EntryEvent event) {
   ...
   if (!event.getOperation().isPutAll()) {
     throwException("afterUpdate invoked,  for " + event.getKey() + "but Operation.isPutAll() returns false");
   }
