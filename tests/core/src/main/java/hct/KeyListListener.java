 ...
 * @since 4.2.3
 */
public class KeyListListener extends util.AbstractListener implements CacheListener, Declarable {
 ...
 *         false otherwise.
 */
public KeyListListener(boolean isCarefulValidation) {
...
/** Check that we're expecting updates on this key
 */
protected void checkEvent(EntryEvent event) {
   Object key = event.getKey();
   //  always allow locally generated events
   if (event.isOriginRemote() == true) {
      if (!this.keyList.contains(key)) {
         throwException("Event received for " + key + " but this client registeredInterest in " + keyList.toString());
      }
   }
} 
...
public void setKeyList( List keyList ) {
   Log.getLogWriter().info("Listener expects events for keyList entries only " + keyList);
