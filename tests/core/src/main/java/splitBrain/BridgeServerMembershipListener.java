...
public class BridgeServerMembershipListener extends AbstractMembershipListener implements BridgeMembershipListener {
  ...
  public void memberJoined(BridgeMembershipEvent event) {
    ...
    if (!event.isClient()) {
       ...
       long count = sc.incrementAndRead( MLRioBB.actualServerJoinedEvents );
  ...
  public void memberLeft(BridgeMembershipEvent event) {
    ...
    if (!event.isClient()) {
       SharedCounters sc = MLRioBB.getBB().getSharedCounters();
       long count = sc.incrementAndRead( MLRioBB.actualServerDepartedEvents);
  ...
  public void memberCrashed(BridgeMembershipEvent event) {
    ...
    if (!event.isClient()) {
       SharedCounters sc = MLRioBB.getBB().getSharedCounters();
       long count = sc.incrementAndRead( MLRioBB.actualServerCrashedEvents);
