...
public class AEQHelper {
  ...
  public static void waitForAsyncEventQueuesToDrainOnce() {
    ...
    Log.getLogWriter().info("Wait for async event queues requested time is " + waitRequestedTime + ", end time is : " + waitEndTime);
    synchronized (AEQHelper.class) {
      if (waitRequestedTime > waitEndTime) { // do the wait
        try {
          ...
          Log.getLogWriter().info("Wait end time is " + waitEndTime);
        }
      } else {
        Log.getLogWriter().info("This thread is not waiting for async event queues to drain because it was done by another thread in this member");
      }
    }
  }
  ...
  public static void waitForAsyncEventQueuesToDrain() {
    ...
    if (theCache == null) {
      Log.getLogWriter().info("No async event queues to wait for; cache is null");
      return;
    }
    ...
    if ((senderSet == null) || (senderSet.size() == 0)) {
      Log.getLogWriter().info("No gateway senders in this member");
    } else {
    ...
    if (queueRegions.size() == 0) {
      Log.getLogWriter().info("No async event queue regions found in this member.");
    }
    ...
    while (waits++ < MAX_WAITS) { 
      ...
      for (Map.Entry<String,Region> entry : queueRegions.entrySet()) {
        ...
        if (!entry.getValue().isDestroyed()) {
          ...
          Log.getLogWriter().info("Queue size for " + regionName + ": " + currentRegionSize);
      ...
      if (allQueuesEmpty) {
        Log.getLogWriter().info("All async event queues are empty");
        break;
      }
    ...
    for (Region aRegion: queueRegions.values()) {
      if (aRegion.getAttributes().getDataPolicy().withPartitioning()) {
        try {
          ParRegUtil.verifyBucketCopies(aRegion, aRegion.getAttributes().getPartitionAttributes().getRedundantCopies());
  ...
  private static List<Region> getGatewaySenderRegions(GatewaySender sender) {
    ...
    //  Log.getLogWriter().info("Concurrent parallel gateway sender " + sender.getId() + " uses region named " + processor.getQueue().getRegion().getFullPath());
    } else {
