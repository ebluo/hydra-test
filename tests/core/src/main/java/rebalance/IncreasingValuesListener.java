 ...
 *
 *  Constraints:
 *  - skips in sequence are permitted, and counts accumulated in the WANRecoveryBB (missedUpdates)
 ...
 * @since 6.0
 */
public class IncreasingValuesListener extends util.AbstractListener implements CacheListener, Declarable {
...
public void afterUpdate(EntryEvent event) {
   ...
   if (newValue.intValue() < oldValue.intValue()) {
      ...
      logCall("afterUpdate", event);
      WANRecoveryBB.getBB().getSharedCounters().add(WANRecoveryBB.missedUpdates, diff-1);
   } else if (diff == 0) {   // duplicate update
      logCall("afterUpdate", event);
      long ctr = WANRecoveryBB.getBB().getSharedCounters().incrementAndRead(WANRecoveryBB.duplicateUpdates);
...
/**
 * Utility method to write an Exception string to the WANRecoveryBB Blackboard and
