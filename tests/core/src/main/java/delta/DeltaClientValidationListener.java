 ...
 * of the data coming via the events. Used by current version VM
 * 
 * @author aingle
 * @since 6.1
 *
 */
public class DeltaClientValidationListener extends CacheListenerAdapter {
  ...
  public void afterUpdate(EntryEvent event) {
    synchronized (latestValues) {
      ...
      boolean isStorageClient = event.getRegion().getAttributes()
          .getDataPolicy().withStorage();
      if (isStorageClient && newValue.getFromDeltaCounter() == 0) {
