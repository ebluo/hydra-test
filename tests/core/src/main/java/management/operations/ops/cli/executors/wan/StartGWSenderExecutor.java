...
public class StartGWSenderExecutor extends AbstractTestCommandExecutor {
 ...
 */
  ...
  public Object verifyCommand(TestableGfsh gfsh, Object object) {
    addToRunningSenderIdList(senderId);
    this.senderId = null;
    return null;
  }
  ...
  protected void fillMandatoryOption(TestCommandInstance instance, String name) {
    if(CliStrings.START_GATEWAYSENDER__GROUP.equals(name)){
      instance.addOption(name, getGroupsNoManaging());
    }else if(CliStrings.START_GATEWAYSENDER__ID.equals(name)){
      this.senderId = getStoppedSenderId();
      instance.addOption(name, senderId);
    }else if(CliStrings.START_GATEWAYSENDER__MEMBER.equals(name)){
