 ...
 */
  ...
  public Object verifyCommand(TestableGfsh gfsh, Object object) {
    addToPausedSenderIdList(senderId);
    this.senderId = null;
    return null;
  }
  ...
  protected void fillMandatoryOption(TestCommandInstance instance, String name) {
    if(CliStrings.PAUSE_GATEWAYSENDER__GROUP.equals(name)){
      instance.addOption(name, getGroupsNoManaging());
    }else if(CliStrings.PAUSE_GATEWAYSENDER__ID.equals(name)){
      senderId = getPausedSenderId();
      instance.addOption(name, senderId);
    }else if(CliStrings.PAUSE_GATEWAYSENDER__MEMBER.equals(name)){
