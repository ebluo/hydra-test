...
public class ResumeGWSenderExecutor extends AbstractTestCommandExecutor {
 ...
 */
  ...
  protected void fillMandatoryOption(TestCommandInstance instance, String name) {
    if(CliStrings.RESUME_GATEWAYSENDER__GROUP.equals(name)){
      instance.addOption(name, getGroupsNoManaging());
    }else if(CliStrings.RESUME_GATEWAYSENDER__ID.equals(name)){
      senderId = getSenderId();
      instance.addOption(name, senderId);
    }else if(CliStrings.RESUME_GATEWAYSENDER__MEMBER.equals(name)){
