...
public class StatusGWSenderExecutor extends AbstractTestCommandExecutor {
 ...
 */
  ...
  protected void fillMandatoryOption(TestCommandInstance instance, String name) {
    if(CliStrings.STATUS_GATEWAYSENDER__GROUP.equals(name)){
      instance.addOption(name, getGroupsNoManaging());
    }else if(CliStrings.STATUS_GATEWAYSENDER__ID.equals(name)){
      instance.addOption(name, getSenderId());
    }else if(CliStrings.STATUS_GATEWAYSENDER__MEMBER.equals(name)){
