...
public class ParallelSenderRegionQueueListener extends SilenceListener {
...
//    super.afterDestroy(event);
    SilenceListenerBB.getBB().getSharedCounters().setIfLarger(SilenceListenerBB.lastEventTime, System.currentTimeMillis());
     ...
     // Based on #44947, following check is commented. 
//    GatewaySender sender = getSenderForEvent(event);
//    if(sender !=null){
//      if(sender.isPaused()){
//        //if event belongs to secondary bucket then skip
//        RegionQueue queue = ((ParallelGatewaySenderImpl)sender).getQueues().toArray(new RegionQueue[1])[0];
//        PartitionedRegion pr = ((PartitionedRegion)((ParallelGatewaySenderQueue)queue).getRegion());
//        int bucketId = (int)(((Long)event.getKey()) / pr.getTotalNumberOfBuckets());
//        BucketRegion br = pr.getBucketRegion(bucketId);
